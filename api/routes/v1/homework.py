...
def create_homework(
    ...
    homework.delivery_date = date.fromisoformat(homework.delivery_date)  # type: ignore
...
def homework_stats(
    ...
    metric: str = Query('count', description='Aggregation metric: count, sum, avg'),
    field: Optional[str] = Query(None, description='Field for sum/avg metrics'),
    session: Session = Depends(get_session),
    ...
    delivery_date: Optional[date] = Query(None, description='Filter by delivery_date'),
) -> Dict[str, Any]:
    ...
    if metric == 'distinct':
        ...
        if group_by:
            results = [{group_by: g, metric: v} for g, v in results]  # type: ignore
        else:
...
@router.get('/homeworks/{homework_id}', response_model=Homework, status_code=200)
def read_homework(
    homework_id: int,
    session: Session = Depends(get_session),
):
...
@router.put('/homeworks/{homework_id}', response_model=Homework, status_code=200)
def update_homework(
    homework_id: int,
    homework: Homework,
    session: Session = Depends(get_session),
