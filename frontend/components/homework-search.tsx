  ...
  React.useEffect(() => {
    async function fetchHomeworks() {
      try {
        ...
        if (filters.grade_id)
          params.set("grade_id", filters.grade_id.toString());
        if (filters.class_id)
          params.set("class_id", filters.class_id.toString());
        const res = await fetch(
          `/api/v1/homeworks/stats?${params.toString()}&group_by=delivery_date&metric=count`,
          { cache: "no-store" },
        );
        if (!res.ok) {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  homeworkCount > 0
                    ? "bg-blue-100 shadow hover:shadow-lg"
                    : "bg-white",
                ...
                onClick={() => {
                  ...
                  router.push(
                    `/homework/list/${encodeURIComponent(formattedDate)}`,
                  );
                }}
              >
                <div
                  className={cn(
                    "mb-2",
                    isToday ? "font-bold text-green-700" : "text-gray-700",
                  )}
                >
                  {dayLabel}
                </div>
