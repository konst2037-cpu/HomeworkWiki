...
export default function HomeworkListPage({ params }: ListPageProps) {
  ...
  React.useEffect(() => {
    ...
    fetch(
      `/api/v1/homeworks/stats?group_by=delivery_date&metric=distinct&${params.toString()}`,
    )
      .then((res) => res.json())
      .then((data) => {
        ...
        {homeworkCount > limit && (
          <div className="flex justify-center md:justify-between items-center gap-4 py-2">
            <div className="inline-flex rounded-md gap-2" role="group">
              ...
              {homeworkCount / limit > 4 ? (
                <>
                  ...
                  {Array.from(
                    {
                      length: Math.min(2, Math.ceil(homeworkCount / limit) - 2),
                    },
                    (_, idx) => {
                      const page = Math.max(3, offset / limit - 2) + idx;

                      // -1 to avoid duplicate number in page button
                      if (page >= Math.ceil(homeworkCount / limit) - 1)
                        return null;
                      ...
                      return (
                        <Button
                          key={page}
                          variant={
                            offset / limit === page ? "default" : "outline"
                          }
                  ...
                  <Button
                    variant={
                      offset / limit === Math.ceil(homeworkCount / limit) - 1
                        ? "default"
                        : "outline"
                    }
                    onClick={() =>
                      setOffset((Math.ceil(homeworkCount / limit) - 1) * limit)
                    }
                ...
                Array.from(
                  { length: Math.ceil(homeworkCount / limit) },
                  (_, i) => (
                    <Button
                      key={i}
            ...
            <span className="text-slate-600 text-sm">
              ...
              {Math.max(1, Math.ceil(homeworkCount / limit))}
            </span>
          </div>
        )}
          ...
          <Table>
            <TableBody>
              {homeworks.map((hw) => (
                ...
                >
                  <TableCell className="font-medium w-[12vw] max-w-[120px] truncate">
                    {hw.subject}
                  </TableCell>
                  <TableCell className="whitespace-normal w-[40vw]">
                    {hw.content}
                  </TableCell>
